...
/**
 * Get RSVP response rows for an invitation (for dashboard ledger)
 */
export const getInvitationResponses = async (req, res) => {
  try {
    const { id } = req.params;
    const userId = req.user.id;
    ...
    const { data: responses, error } = await supabaseAdmin
      .from("rsvp_responses")
      .select(
        "id, guest_name, guest_email, rsvp_status, dietary_requirements, additional_guests, created_at",
      )
      .eq("invitation_id", id)
      .order("created_at", { ascending: false });
