...
const refs = {
  ...
  previewPanel: document.querySelector("#previewModal .modal-panel"),
  previewBackdrop: document.querySelector("#previewModal .modal-backdrop"),
...
if (typeof window !== "undefined") {
  window.TEMPLATE_CATALOG = TEMPLATE_CATALOG.map(normalizeTemplate).filter(
    (item) => item.id,
  );
}
...
function normalizeTemplate(template, sourceOrder = 0) {
  const normalizedPrice = Number(template.price);
  const rawId = String(template.id || "");
  const thumbnailPath = rawId
    ? `assets/images/thumbnails/${rawId}.webp`
...
function sortTemplatesForMarketplace(templates) {
  return [...templates].sort((a, b) => {
    const categoryA = CATEGORY_PRIORITY[a.category] ?? 99;
    const categoryB = CATEGORY_PRIORITY[b.category] ?? 99;
...
function applyFilters() {
  ...
  if (state.activeCategory === "all" || state.activeCategory === "trending") {
    state.filtered = sortTemplatesForMarketplace(filtered);
  } else {
    state.filtered = filtered.sort(
...
function renderGrid() {
  ...
  state.filtered.forEach((template, index) => {
    ...
    card.style.setProperty("--card-stagger", `${Math.min(index, 10) * 55}ms`);
    card.setAttribute("data-aos", "fade-up");
    card.setAttribute("data-aos-duration", "560");
    card.setAttribute("data-aos-delay", String(Math.min(index, 8) * 45));
...
function openPreview(template) {
  ...
  // Defensive fallback: if any stale/overridden CSS hides the panel,
  // force it visible so users always see preview content above the backdrop.
  if (refs.previewPanel) {
    refs.previewPanel.style.removeProperty("display");
    refs.previewPanel.style.removeProperty("visibility");
    refs.previewPanel.style.removeProperty("opacity");
    refs.previewPanel.style.removeProperty("z-index");
  }

  if (refs.previewBackdrop) {
    refs.previewBackdrop.style.removeProperty("z-index");
  }
...
function maybeShowAuthPrompt() {
  if (FLOW_GUEST_STUDIO_ENABLED) {
    ...
    if (url.searchParams.has("authPrompt")) {
      url.searchParams.delete("authPrompt");
